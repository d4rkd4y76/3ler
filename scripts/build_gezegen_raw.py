#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build gezegen-gemini-raw.json from parent transcript user message."""
...
def main() -> None:
    line = TRANSCRIPT.read_text(encoding="utf-8").splitlines()[88]
    obj = json.loads(line)
    ...
    for q in items:
        ...
        if qid in TEXT_FIXES:
            q["text"] = TEXT_FIXES[qid]
        ordered.append(
            {
                "id": q["id"],
                "level": q["level"],
