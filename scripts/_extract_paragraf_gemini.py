...
OUT = Path(__file__).resolve().parents[1] / "data" / "paragraf-gemini-raw.json"

raw = None
for line in TRANSCRIPT.open(encoding="utf-8"):
    if "paragraf3_001" not in line:
        continue
    obj = json.loads(line)
    ...
    if "Paragrafta anlam" not in text and "PARAGRAFTA" not in text.upper():
        continue
    m = re.search(r"\[\s*\n?\s*\{", text)
    if not m or "paragraf3_001" not in text[m.start() : m.start() + 500]:
        continue
    chunk = text[m.start() :]
    depth = 0
    end = 0
