#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Gemini'den gelen Cümlede Anlam (Sebep-Sonuç) sorularını işler."""
...
TOPIC = get_topic("t05")
RAW_PATH = ROOT / "data" / "cumle-gemini-raw.json"
...
def fix_premise_text_split(q: dict) -> dict:
    for key in ("text", "correct", "wrong1", "wrong2", "explanation", "premise"):
        if q.get(key):
            q[key] = strip_markdown(str(q[key])).strip() or None
    ...
    if premise and text.startswith("Bu cümlede"):
...
def prepare_question(q: dict) -> dict:
    ...
    if q.get("premise") and q.get("text"):
        ...
        if "Yukarı" not in t and "yukarı" not in t.lower():
            if t.startswith("Aşağıdaki"):
                q["text"] = "Yukarıdaki cümlede " + t[0].lower() + t[1:]
            elif len(q["premise"]) > 15:
                q["text"] = "Yukarıdaki cümlede " + t[0].lower() + t[1:]
    if q.get("text"):
