#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tek bir 3. sınıf Fen Bilimleri konusunu Firebase + Bunny'ye yükler."""
...
from s3_fen_topics import TOPICS, get_topic, HEADING_ID, LESSON_ID
...
def push_one(cfg: dict) -> None:
    topic = cfg["id"]
    topic_base = f"championData/headings/{HEADING_ID}/lessons/{LESSON_ID}/topics/{topic}"
    ...
    tmp = ROOT / "dist" / f"_push_fen_{topic}.json"
    ...
    name_body = json.dumps(cfg["name"], ensure_ascii=False).encode("utf-8")
    bunny_put(host, zone, api_key, f"v{version}/rtdb/{topic_base}/name.json", name_body)
    order_body = json.dumps(cfg["order"], ensure_ascii=False).encode("utf-8")
    bunny_put(host, zone, api_key, f"v{version}/rtdb/{topic_base}/order.json", order_body)
    print(f"Bunny: {topic} tamam.")


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("topic", nargs="?", help="t01..t07 veya 'all'")
    args = ap.parse_args()

    targets = TOPICS if (not args.topic or args.topic == "all") else [get_topic(args.topic)]
    for cfg in targets:
        if not (DATA_DIR / cfg["data_file"]).is_file():
            print(f"Atlanıyor (paket yok): {cfg['id']} — {cfg['data_file']}")
            continue
        push_one(cfg)
