#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Kelime Dağarcığı 150 soruyu slug topic'ten t02'ye taşır, duplicate topic siler."""
...
CORRECT_TOPIC = "t02"
WRONG_TOPIC = "topic_02_kelime_dagarcigi_es_ve_zit_anlam"
...
def patch_dllwrld() -> None:
    ...
    slug = topics.pop(WRONG_TOPIC, None)
    if slug:
        topics[CORRECT_TOPIC] = slug
        topics[CORRECT_TOPIC]["name"] = (
            "Kelime Dağarcığı (Eş ve zıt anlamlı kelimeler, Eş Sesli/Sesteş kelimeler)"
        )
        topics[CORRECT_TOPIC]["order"] = 2
        topics[CORRECT_TOPIC]["active"] = True
    dllwrld.write_text(json.dumps(data, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    print(f"dllwrld.json: {WRONG_TOPIC} -> {CORRECT_TOPIC}")
...
def main() -> int:
    ...
    tmp = ROOT / "dist" / "_migrate_kelime_t02.json"
    ...
    print("Firebase: t02 güncellendi")
    ...
    for qid in payload["questions"]:
        bunny_delete(host, zone, api_key, f"v{version}/rtdb/{WRONG_BASE}/questions/{qid}.json")
    for suffix in ("questionIds.json", "active.json"):
        bunny_delete(host, zone, api_key, f"v{version}/rtdb/{WRONG_BASE}/{suffix}")

    print(f"Bunny: {n} soru t02'ye taşındı, slug topic CDN temizlendi")
    patch_dllwrld()
    return 0
