...
(function(){
  ...
  async function openFillBlankScreen(){
    ...
    if (attemptSnap.exists()){
      if (typeof showAlert === 'function') showAlert('🧩 Günlük Hak Kullanıldı\nBugünkü Boşluk Ustası hakkını kullandın. Yarın yeni meydan okuma seni bekliyor!');
      return;
    }
  ...
  function ensureUI(){
    ...
    st.textContent = `
      ...
      #fillblank-screen{display:none;position:fixed;inset:0;z-index:100001;background:
        ...
        linear-gradient(180deg,#7c3aed 0%, #4f46e5 45%, #22d3ee 100%);color:#0f172a;align-items:center;justify-content:center}
      .fb-card{width:min(760px,92vw);background:linear-gradient(180deg,#ffffff,#f8fbff 55%,#eff6ff);border:1px solid rgba(255,255,255,.72);border-radius:22px;padding:22px;box-shadow:0 24px 60px rgba(30,41,59,.35), inset 0 1px 0 rgba(255,255,255,.9);position:relative;overflow:hidden}
