...
window.onload = async () => {
    try {
            ...
            selectionClassSelect.addEventListener('change', () => {
                if (selectionClassSelect.value === "") {
                    if (selectionNameInput) {
                        ...
                        if (!Number(window.NOVA_LOGIN_FIXED_GRADE || 0)) {
        ...
        if (studentPasswordInput) {
            studentPasswordInput.addEventListener('input', checkLoginButtonState);
            studentPasswordInput.addEventListener('change', checkLoginButtonState);
        }

        const storedStudent = localStorage.getItem('selectedStudent');
        const tabAlive = typeof window.novaHasTabSession === 'function' ? window.novaHasTabSession() : false;
        if (storedStudent && tabAlive) {
            ...
            try { if (typeof window.novaMarkTabSession === 'function') window.novaMarkTabSession(); } catch (_) {}
            ...
            try {
              if (typeof window.novaLobbySyncBirlestir === 'function') window.novaLobbySyncBirlestir();
            } catch (_) {}
            ...
            try {
              if (typeof window.novaApplyRememberedLogin === 'function') window.novaApplyRememberedLogin();
            } catch (_) {}
        }
...
async function handleLogin() {
    ...
    try {
        ...
        /* Beni hatırla: şifre alanını temizlemeden ÖNCE kaydet */
        try {
          ...
          if (remEl && remEl.checked) {
            ...
            if (typeof window.novaWriteLoginPref === 'function') window.novaWriteLoginPref(pref);
            else localStorage.setItem('duello_login_pref', JSON.stringify(pref));
          } else if (typeof window.novaWriteLoginPref === 'function') {
            window.novaWriteLoginPref({ user: '', pass: '', cls: '', rem: false });
          }
        } catch (_) {}

        try {
          document.documentElement.classList.add('nova-has-session', 'nova-main-screen-visible', 'nova-boot-pending');
          document.documentElement.classList.remove('nova-guest-boot');
        ...
        try {
          document.dispatchEvent(new CustomEvent('duello:loginSuccess'));
        } catch (_) {}
        try {
          if (typeof window.novaLobbySyncBirlestir === 'function') window.novaLobbySyncBirlestir();
        } catch (_) {}
...
selectionClassSelect.addEventListener('change', () => {
    if (selectionClassSelect.value === "") {
        selectionNameInput.disabled = true;
        /* Sabit sınıf modunda kullanıcı adını silme — Beni hatırla bozulmasın */
        if (!Number(window.NOVA_LOGIN_FIXED_GRADE || 0)) {
            selectionNameInput.value = "";
        }
    } else {
...
function checkLoginButtonState() {
    try {
        if (selectionClassSelect && typeof window.novaLockSelectToFixedGrade === 'function') {
            if (!(selectionClassSelect.value || '').trim()) {
                window.novaLockSelectToFixedGrade(selectionClassSelect, window.NOVA_LOGIN_FIXED_GRADE || 1);
            }
        }
    } catch (_) {}
    /* Kids login kendi validate'ini kullanıyorsa ona bırak */
    if (typeof window.novaSyncLoginCta === 'function') {
        try { window.novaSyncLoginCta(); } catch (_) {}
        return;
    }
    var classOk = !!(selectionClassSelect && (selectionClassSelect.value || '').trim());
    var userOk = !!(selectionNameInput && (selectionNameInput.value || '').trim());
    var passOk = !!(studentPasswordInput && (studentPasswordInput.value || '').trim());
    if (classOk && userOk && passOk) {
        ...
        if (studentPasswordInput) {
            studentPasswordInput.disabled = !userOk;
        }
    }
}
...
logoutButton.addEventListener('click', async () => {
   try {
       ...
       try {
         if (typeof window.novaClearTabSession === 'function') window.novaClearTabSession();
       } catch (_) {}
       try {
...
async function fetchClassesForSelection() {
    ...
    try{
        ...
        window.__novaClassesFetchedForLogin = true;
        try {
          document.dispatchEvent(new CustomEvent('nova:classes-ready'));
        } catch (_) {}
        try {
          if (typeof window.novaApplyRememberedLogin === 'function') window.novaApplyRememberedLogin();
        } catch (_) {}
  ...
  try {
    ...
    (async function () {
        ...
        async function loadRanking(opts) {
            ...
            if (__rankingLoadInFlight) {
                const waitStart = Date.now();
                while (__rankingLoadInFlight && (Date.now() - waitStart) < 12000) {
            ...
            const now = Date.now();
            if (!force && (now - __rankingLastLoadTs) < RANKING_NET_DEBOUNCE_MS) return;
        ...
        try { window.loadRanking = loadRanking; } catch (_) {}
