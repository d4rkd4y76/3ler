...
(function () {
  ...
  async function refreshMainScreenHeroCore() {
    ...
    try {
      ...
      if (cache && cacheAt && Date.now() - cacheAt < 120000) {
        ...
        while (!cache && Date.now() - waitStart < 400) {
      ...
      /* True-clip paketleri idle'da — açılışta bağlantı sırasını doldurmasın */
      function deferTrueClips(fn) {
      ...
      if (heroId === 'firtina_okcu') deferTrueClips(window.novaFirtinaOkcuPreloadTrueClipsIfEquipped);
      if (heroId === 'star_fairy') deferTrueClips(window.novaYildizPerisiPreloadTrueClipsIfEquipped);
      if (heroId === 'tas_muhafiz') deferTrueClips(window.novaTasMuhafizPreloadTrueClipsIfEquipped);
      if (heroId === 'golge_parsi') deferTrueClips(window.novaGolgeParsiPreloadTrueClipsIfEquipped);
      if (heroId === 'bilge_baykus') deferTrueClips(window.novaBilgeBaykusPreloadTrueClipsIfEquipped);
      if (heroId === 'buz_ejder') deferTrueClips(window.novaBuzEjderPreloadTrueClipsIfEquipped);
      if (heroId === 'alev_ejder') deferTrueClips(window.novaAlevEjderPreloadTrueClipsIfEquipped);
      if (heroId === 'gece_ejder') deferTrueClips(window.novaGeceEjderPreloadTrueClipsIfEquipped);
    } catch (_) {
      clearMainHeroSlot(slot);
    }
  }
  ...
  async function refreshMainScreenHero(opts) {
    ...
    /* Aynı anda birden fazla hero refresh = çift sprite indirme + Firebase sırası */
    if (window.__novaMainHeroRefreshInFlight) {
      return window.__novaMainHeroRefreshInFlight;
    }
    ...
    function runCore() {
      if (myGen !== mainHeroRefreshGen) return Promise.resolve();
      return refreshMainScreenHeroCore();
    }

    window.__novaMainHeroRefreshInFlight = (delay
      ? new Promise(function (resolve) {
          mainHeroRefreshTimer = setTimeout(function () {
      ...
      window.__novaMainHeroRefreshInFlight = null;
    });
