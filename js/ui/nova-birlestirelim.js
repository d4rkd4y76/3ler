...
(function () {
  ...
  function pinBirlesViewBoot(title, text) {
    var body = document.getElementById("birles-body");
    if (!body) return;
    body.classList.add("is-view-booting");
    if (!document.getElementById("birles-view-boot")) {
      body.insertAdjacentHTML("afterbegin", birlesViewBootHtml(title, text));
    }
  }
  ...
  function clearBirlesViewBootFlag() {
    ...
    var boot = document.getElementById("birles-view-boot");
    if (boot) boot.remove();
  }

  function waitBirlesPaintSettle(minMs) {
    minMs = minMs == null ? 220 : minMs;
    return new Promise(function (resolve) {
      requestAnimationFrame(function () {
        requestAnimationFrame(function () {
          setTimeout(resolve, Math.max(0, minMs));
        });
      });
    });
  }

  async function finishBirlesViewBoot(bootTok, title, text, renderFn, settlePromise) {
    if (!isBirlesViewBootCurrent(bootTok)) return false;
    try {
      if (typeof renderFn === "function") renderFn();
    } catch (_) {}
    pinBirlesViewBoot(title || "Ses Evreni", text || "Hazırlanıyor…");
    var waits = [waitBirlesPaintSettle(240)];
    if (settlePromise && typeof settlePromise.then === "function") {
      waits.push(
        Promise.race([
          settlePromise.catch(function () {}),
          waitMinMs(7000)
        ])
      );
    }
  ...
  function openGroups() {
    ...
    Promise.all([ensureBirlesProgress(), ensureMedia(), waitMinMs(220)])
      .then(function () {
        ...
        return finishBirlesViewBoot(bootTok, "Ses Evreni", "Açılıyor…", function () {
          renderGroups();
        });
      })
      .catch(function () {
    ...
    Promise.all([ensureBirlesProgress(), ensureMedia(), waitMinMs(260)])
      .then(function () {
        ...
        return finishBirlesViewBoot(
          ...
          revealHubAfterVideo(".birles-hub--flower", ".birles-flower__mascot")
        );
      })
      .catch(function () {
        if (!isBirlesViewBootCurrent(bootTok) || view !== "letters") return;
        return finishBirlesViewBoot(bootTok, "Ses Evreni", "Grup açılıyor…", function () {
          renderLetters();
        });
      });
  }
    ...
    Promise.all([ensureBirlesProgress(), ensureMedia(), waitMinMs(280)])
      .then(function () {
        ...
        return finishBirlesViewBoot(
          ...
          revealHubAfterVideo(".birles-sound-view", ".birles-ses-evreni__vid")
        );
      })
      .then(function (ok) {
        if (!ok) return;
      ...
      .catch(function () {
        if (!isBirlesViewBootCurrent(bootTok) || view !== "sound") return;
        return finishBirlesViewBoot(bootTok, sound.title || "Ses", "Ses açılıyor…", function () {
          renderSound(sound);
        });
      });
  }
  ...
  function renderLaneGallery(sound) {
    ...
    Promise.all([ensureMedia(), waitMinMs(200)])
      .then(function () {
        ...
        return finishBirlesViewBoot(bootTok, activeLane.title || "Galeri", "Galeri açılıyor…", function () {
    ...
    ensureMedia()
      .then(function () {
        ...
        return finishBirlesViewBoot(
          ...
          if (!ok) return false;
          if (view !== "play" || activeFusion !== fusion) return false;
  ...
  async function playPoolFinale(finalWord, soundKey, vv, token) {
    ...
    if (token !== animToken) return;
    /* Cümle: üstte tik paneli var — patlamada ekstra metin overlay gösterme */
    if (burst && !isCumleFinale) {
      burst.textContent = String(finalWord || "");
      burst.hidden = false;
      burst.classList.remove("is-rise", "is-boom-in");
      void burst.offsetWidth;
      burst.classList.add("is-boom-in");
    } else if (burst && isCumleFinale) {
      burst.hidden = true;
      burst.textContent = "";
      burst.classList.remove("is-rise", "is-boom-in");
    }
  ...
  function setHeceActive(sylIndex, boardId) {
    ...
    var isCumle = !!(root.closest && root.closest(".birles-play--cumle"));
    root.querySelectorAll(".birles-hece-seg").forEach(function (el) {
      var on = String(el.getAttribute("data-syl")) === String(sylIndex);
      el.classList.toggle("is-on", on);
      /* Kelime/Cümle: önceki hecede mint ton bırakma — sadece aktif yeşil çizgi */
      if (isKelime || isCumle) {
  ...
  async function runSentenceAnimation(sound, fusion, token) {
    ...
    function cumleMidHeceHtml(wordText, displaySyls) {
      ...
      for (var si = 0; si < syls.length; si++) {
        if (si > 0) {
          chips.push('<span class="birles-cumle-mid__dot" aria-hidden="true">·</span>');
        }
        chips.push(
          '<span class="birles-cumle-mid__syl birles-hece-seg" data-syl="' +
            si +
            '">' +
            esc(syls[si]) +
            "</span>"
    ...
    function showCumleCompleteBox() {
      ...
      stage.innerHTML =
        ...
        '<div class="birles-cumle-done__burst" aria-hidden="true"></div>' +
        '<div class="birles-cumle-done__tick" aria-hidden="true">' +
        '<span class="birles-cumle-done__ring"></span>' +
        '<svg viewBox="0 0 52 52" width="88" height="88" focusable="false">' +
        '<circle cx="26" cy="26" r="24" fill="none"/>' +
        '<path d="M14.5 27.2 L22.2 34.6 L37.8 17.8" fill="none"/>' +
        "</svg></div>" +
        '<p class="birles-cumle-done__label">Süper! Cümle doğru</p>' +
    ...
    stage.classList.add("is-quiet");
    stage.innerHTML =
      '<div class="birles-cumle-mid birles-cumle-mid--idle" aria-hidden="true">' +
      '<p class="birles-cumle-mid__cap">Cümleyi dinle</p></div>';
    ...
    for (var wi = 0; wi < words.length; wi++) {
      if (token !== animToken) return;
      if (!(await teachWordBySyllables(wi, words[wi]))) return;
    }
