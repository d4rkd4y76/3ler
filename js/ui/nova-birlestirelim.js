...
(function () {
  ...
  function openHub() {
    ...
    setHeader(
      "BİRLEŞTİRELİM",
      (DD && DD.GROUP1_SUBTITLE) || "A · N · E · T · İ · L",
      (DD && DD.GROUP1_TAGLINE) || "Maarif Modeli"
    );
    renderHub();
    var vv = voice();
    if (vv) vv.unlock();
  ...
  function renderHub() {
    ...
    if (!body) return;
    var DD = data();
    var sounds = (DD && DD.GROUP1_SOUNDS) || [];
    var html =
      '<div class="birles-hub">' +
      ...
      "      <strong>Hadi birleştirelim!</strong>" +
      "      <span>Ses kartlarına dokun, dinle, birleştir. Neşeli okuma macerası başlıyor!</span>" +
      "    </div>" +
      "  </div>" +
      '  <p class="birles-path-label">1. grup · sırayla seç</p>' +
      '  <div class="birles-sound-rail" role="list">';

    sounds.forEach(function (s, idx) {
      html +=
        '<button type="button" class="birles-sound-chip" role="listitem" data-sound="' +
        esc(s.id) +
        '" style="--chip:' +
        esc(s.color) +
        ";--glow:" +
  ...
  function openSound(soundId) {
    ...
    activeFusion = null;
    setBack(true);
    setHeader(sound.title, "Kartlara dokun, birleştir!", "1. Grup · " + sound.displayUpper);
    renderSound(sound);
    var vv = voice();
    if (vv) vv.unlock();
  ...
  function renderSound(sound) {
    ...
    (sound.fusions || []).forEach(function (f) {
      ...
      var thumb =
        med && med.imageUrl
          ? '<img class="birles-fusion-card__img" src="' + esc(med.imageUrl) + '" alt="" loading="lazy" />'
          : '<span class="birles-fusion-card__badge">' + esc((f.kind || "hece").toUpperCase()) + "</span>";
      html +=
        '<button type="button" class="birles-fusion-card" data-fusion="' +
        esc(f.id) +
        '">' +
  ...
  async function stageMerge(left, right, token) {
    ...
    var merged = right != null ? String(left) + String(right) : String(left);
  ...
  function injectEntryButton(listEl) {
    ...
    btn.innerHTML =
      ...
      '<span class="roborox-topic-item__meta">1. grup sesler · A N E T İ L</span>' +
      "</span>" +
  ...
  window.novaOpenBirlestirelim = openHub;
