...
(function () {
  ...
  async function openEggScreen() {
    ...
    try {
      ...
      /* Ekran açık ve boyandıktan sonra prep kalksın */
      if (eggScreenOpen) {
        try {
          await new Promise(function (r) {
            requestAnimationFrame(function () {
  ...
  /**
   * Karışık (fire/ice/night rastgele) yumurta torbası.
   * Tip başına en fazla 20’lik chunk (novaGrantDragonEgg cap).
   * @returns {{ ok: boolean, bag: {fire:number,ice:number,night:number}, total: number }}
   */
  window.novaGrantMixedDragonEggs = async function (total, opts) {
    opts = opts || {};
    total = Math.max(1, Math.min(999, Number(total) || 1));
    var bag = { fire: 0, ice: 0, night: 0 };
    for (var i = 0; i < total; i++) {
      bag[EGG_TYPES[Math.floor(Math.random() * EGG_TYPES.length)]]++;
    }
    var ok = true;
    for (var t = 0; t < EGG_TYPES.length; t++) {
      var type = EGG_TYPES[t];
      var left = bag[type];
      while (left > 0) {
        var chunk = Math.min(20, left);
        var granted = await window.novaGrantDragonEgg(type, chunk, {
          suppressToast: true
        });
        if (!granted) {
          ok = false;
    ...
    if (ok && !opts.suppressToast) {
      showEarnToast('fire', total);
    }
    return { ok: ok, bag: bag, total: total };
  };
